...
//  obj.call = function (action, query, callback) {
		...
		if (action === 'SubmitFeed') {
			...
			xml = xml.replace(/’/g, "'");
			xml = iconv.encode(xml, 'iso-8859-1');
//---------------------------------  console.log -------------------------------
...
//      };
		var req = obj.connection.request(options, function (res) {
			...
			res.addListener('data', function (chunk) {
				// If encoding iso-8859-1 return by Amazon...
				if (req.res.headers['content-type'] === 'application/octet-stream')
					data += iconv.decode(chunk, 'iso-8859-1').toString();
				else
					data += chunk.toString()
			});
			res.addListener('end', function() {
				...
				// <HACK> Modded by Romuald (reason: sax can't handle ... OK !)
				// This hack will not affect XML behaviour. Don't worry !
				try {
